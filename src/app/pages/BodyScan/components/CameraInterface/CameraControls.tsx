        ...
        <ConditionalMotion
          ...
          style={{
            background: isCapturing
              ? 'linear-gradient(135deg, var(--brand-warning), var(--brand-warning-dark))'
        ...
        >
          {isCapturing ? (
            <>
              <ConditionalMotion
                ...
                animate={performanceConfig.enableCSSAnimations ? { scale: [1, 0.7, 1] } : { scale: 1 }}
                transition={performanceConfig.enableFramerMotion ? { duration: 0.6, repeat: Infinity, ease: "easeInOut" } : undefined}
              />
              <ConditionalMotion
                className="absolute inset-0 rounded-full border-4 border-brand-warning/60"
                animate={performanceConfig.enableCSSAnimations ? { scale: [1, 1.3, 1], opacity: [0.8, 0.2, 0.8] } : { scale: 1, opacity: 0.5 }}
          ...
          {!isCapturing && performanceConfig.enablePulseAnimations && (
            <ConditionalMotion
              ...
              animate={performanceConfig.enableCSSAnimations ? { scale: [1, 1.1, 1], opacity: [0.5, 0.8, 0.5] } : { scale: 1, opacity: 0.5 }}
              transition={performanceConfig.enableFramerMotion ? { duration: 2, repeat: Infinity, ease: "easeInOut" } : undefined}
            />
          )}
        </ConditionalMotion>
